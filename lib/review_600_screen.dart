...
class Review600Screen extends StatefulWidget {
  const Review600Screen({Key? key}) : super(key: key);

  @override
  State<Review600Screen> createState() => _Review600ScreenState();
}

class _Review600ScreenState extends State<Review600Screen> {
  int currRating = 5;

  Widget _buildRatingStar(int index) {
    if (index < currRating) {
      return const Icon(Icons.star,
          color: ThemeConfig.yellowFadeColor, size: 60);
    }

    return const Icon(Icons.star,
        color: ThemeConfig.greyBlueColor, size: 60);
  }
  ...
  Widget _buildStar() {
    ...
    final stars = List<Widget>.generate(
        ...
        (index) => Container(
              ...
              child: _likeButton(index, _soundController, 35),
            ));
